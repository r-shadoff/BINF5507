...
def normalize_data(data, method='minmax'):
    ...
    if method == 'minmax':
        data_numeric = pd.DataFrame((MinMaxScaler(copy=False).fit(numerical_cols).transform(numerical_cols)), columns = numerical_cols.columns)
        data_combined = pd.concat([data_numeric, data_nonnumeric], axis = 1)
        return data_combined
    elif method == 'standard':
        data_numeric = pd.DataFrame((StandardScaler(copy=False).fit(numerical_cols).transform(numerical_cols)), columns = numerical_cols.columns)
        data_combined = pd.concat([data_numeric, data_nonnumeric], axis = 1)
        return data_combined
    else:
