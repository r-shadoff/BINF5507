...
def impute_missing_values(data, strategy='mean'):
    ...
    for x in columnlist:
        if strategy == 'mean':
            ...
            print("ERROR! Enter mean, median, or mode as the imputation strategy.")
    return data
...
def normalize_data(data, method='minmax'):
    ...
    numerical_cols = data.select_dtypes(include='number')
    numerical_cols_2 = numerical_cols.drop(['target'], axis=1)
    data_nonnumeric = data.select_dtypes(exclude='number')
    target = pd.DataFrame(data["target"])
    #nonnumerical_cols = data.select_dtypes(exclude='number')
    if method == 'minmax':
        data_numeric = pd.DataFrame((MinMaxScaler(copy=False).fit(numerical_cols_2).transform(numerical_cols_2)), columns = numerical_cols_2.columns)
        data_combined = pd.concat([target, data_numeric, data_nonnumeric], axis = 1)
        return data_combined
    elif method == 'standard':
        data_numeric = pd.DataFrame((StandardScaler(copy=False).fit(numerical_cols_2).transform(numerical_cols_2)), columns = numerical_cols_2.columns)
        data_combined = pd.concat([target, data_numeric, data_nonnumeric], axis = 1)
        return data_combined
    else:
        return print("ERROR! Enter minmax or standard as the normalization strategy.")
...
def remove_redundant_features(data, threshold=0.9):
    ...
    :return: pandas DataFrame
    """
    numerical_cols = pd.DataFrame(data).select_dtypes(include='number')
    corr_matrix = numerical_cols.corr().abs()
    upper_tri = corr_matrix.where(np.triu(corr_matrix, k =1).astype(bool)) # k=1 excludes the diagonal from the correlation matrix
    drop_cols = [column for column in upper_tri.columns if any(upper_tri[column] > threshold)]
    dropped_data = data.drop(drop_cols, axis = 1)
    return dropped_data
